...
function addItem() {
    ...
    .then(() => {
        ...
        displayMovies(); // Optional: Refresh the movie list
    })
    .catch((error) => {
...
function displayMovies() {
    const userId = sessionStorage.getItem("uid"); // Logged-in user's UID
    db.collection("movies")           
    .where("userId", "==", userId) // Filter movies by userId
...
firebase.auth().onAuthStateChanged(function(user) {  
    if (user) {
        // User is signed in
        document.getElementById('logged-out-links').style.display = 'none'; // Hide login/register links
        document.getElementById('logged-in-links').style.display = 'block'; // Show logout and username/email links
        
        // Display user's name or fallback to "User" if no display name is set
        ...
        document.getElementById('logged-out-links').style.display = 'block'; // Show login/register links
        document.getElementById('logged-in-links').style.display = 'none'; // Hide logout links
        document.getElementById('welcome-message').textContent = ''; // Clear the welcome message

    }

    const email = user.email || "User"; // Get the user's email (or fallback to "User")
document.getElementById('welcome-message').textContent = `Welcome, ${email}`; // Display the email in the welcome message
